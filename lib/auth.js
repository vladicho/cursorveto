...
function handleAdminApi(request, response, url) {
  ...
  try {
    ...
    // ── CRÉDITOS EXTRAS ──────────────────────────────────────────────────────
    // GET /api/admin/users/:id/credits → status atual de uso do usuário
    ...
    // POST /api/admin/users/:id/credits { credits: N }  → adiciona N créditos extras
    ...
    // DELETE /api/admin/users/:id/credits → zera créditos extras
...
/**
 * Rotas de controle de nesting.
 *   GET  /api/nesting/status  → status de uso do dia
 *   POST /api/nesting/check   → verifica + registra uso { pieceCount: N }
 */
function handleNestingApi(request, response, url) {
  ...
  try {
    ...
    // Sempre busca o usuário fresco do disco (para ter dados atualizados)
