...
function isPublicPath(pathname) {
  ...
  // Webhook do WhatsApp (Meta precisa acessar sem login)
  if (pathname === "/webhook/whatsapp") return true;
  return false;
}
...
function handleAdminApi(request, response, url) {
  ...
  try {
    ...
    // ── BLOQUEAR usuário (muda status para "rejected") ──
    const blockMatch = url.pathname.match(/^\/api\/admin\/users\/([^/]+)\/block$/);
    if (blockMatch && request.method === "POST") {
      ...
      if (target.id === admin.id) {
        sendJson(response, 400, { ok: false, error: "Voce nao pode bloquear a si mesmo." });
        return true;
      }
      const updated = setUserStatus(userId, "rejected");
      sendJson(response, 200, { ok: true, user: publicUser(updated), message: "Usuario bloqueado." });
      return true;
    }

    // ── DESBLOQUEAR usuário (muda status para "approved") ──
    const unblockMatch = url.pathname.match(/^\/api\/admin\/users\/([^/]+)\/unblock$/);
    if (unblockMatch && request.method === "POST") {
      const userId = decodeURIComponent(unblockMatch[1]);
      const target = findUserById(userId);
      if (!target) {
        sendJson(response, 404, { ok: false, error: "Usuario nao encontrado." });
        return true;
      }
      const updated = setUserStatus(userId, "approved");
      sendJson(response, 200, { ok: true, user: publicUser(updated), message: "Usuario desbloqueado." });
      return true;
    }
    ...
    if (deleteMatch && request.method === "DELETE") {
      ...
      if (target.id === admin.id) {
        sendJson(response, 400, { ok: false, error: "Voce nao pode deletar a si mesmo." });
        return true;
      }
      const removed = store.deleteById(userId);
      if (!removed) {
        sendJson(response, 500, { ok: false, error: "Erro ao deletar usuario." });
        return true;
      }
      sendJson(response, 200, { ok: true, message: "Usuario deletado permanentemente." });
      return true;
    }
