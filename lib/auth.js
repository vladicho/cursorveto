...
function authDiagnostics() {
  try {
    ...
    console.error("Diagnostico: falha ao sincronizar bootstrap:", error.message);
  }
  const users = listUsers();
  return {
    bootstrapEmailConfigured: Boolean(process.env.MOLDELAB_BOOTSTRAP_EMAIL),
    bootstrapPasswordConfigured: Boolean(process.env.MOLDELAB_BOOTSTRAP_PASSWORD),
    bootstrapEmail: process.env.MOLDELAB_BOOTSTRAP_EMAIL ? normalizeEmail(process.env.MOLDELAB_BOOTSTRAP_EMAIL) : null,
    userCount: users.length,
    approvedAdminCount: users.filter((user) => isAdmin(user)).length,
...
function handleAuthApi(request, response, url) {
  ...
  try {
    ...
    if (url.pathname === "/api/auth/diagnostics" && request.method === "GET") {
      syncBootstrapAdmin();
      return sendJson(response, 200, { ok: true, diagnostics: authDiagnostics() });
    }
