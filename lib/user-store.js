...
function createUserStore(dataDir) {
  ...
  return {
    ...
    /**
     * Atualiza apenas os campos de nesting_usage e extra_credits do usuário.
     * Faz merge seguro para não sobrescrever outros campos.
     */
    updateNestingData(id, nestingUsage, extraCredits) {
      const users = readStore();
      const index = users.findIndex((user) => user.id === id);
      if (index < 0) return null;
      users[index] = {
        ...users[index],
        nesting_usage: nestingUsage,
    ...
    ensureBootstrapAdmin({ email, password, name, hashPassword }) {
      ...
      if (existing) {
        return this.update(existing.id, {
          name: name || existing.name || "Administrador",
